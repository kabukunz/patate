...
public:
    inline void initGL(bool revertY = false, unsigned int slices = 32);
...
template < class DataPoint, class _WFunctor, typename T>
void
CircleGL<DataPoint, _WFunctor, T>::initGL(bool revertY, unsigned int slices){
    ...
    Eigen::Vector2f data [_slices];
    const double step   =  2.f*M_PI / (_slices-1);
    ...
    for (int i = 0; i != _slices; i++){
        ...
        if (revertY) current(1) = 1.f - current(1);
    }
...
CircleGL<DataPoint, _WFunctor, T>::displayGL() const{
    ...
    glDrawArrays(GL_LINE_STRIP, 0, _slices);
    glBindBuffer(GL_ARRAY_BUFFER,0);
