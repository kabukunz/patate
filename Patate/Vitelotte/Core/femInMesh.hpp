...
bool FemInMesh::compVx(Vertex _v0, Vertex _v1) const
{
...
void FemInMesh::setSingular(Face _f, bool _singular)
{
    assert(!_singular || !m_fFlat[_f]);
    m_fSingular[_f] = _singular;
}

void FemInMesh::setFlat(Face _f, bool _flat)
{
    assert(!m_fSingular[_f] || !_flat);
    m_fFlat[_f] = _flat;
}

unsigned FemInMesh::sizeElement() const //old size()
{
    switch(m_facesType)
    {
...
unsigned FemInMesh::singularVertex(Face _f) const
{
    assert(m_fSingular[_f]);
    return m_fSingularVertex[_f];
}

void FemInMesh::setSingularVertex(Face _f, unsigned _singular)
{
    assert(m_fSingular[_f] && _singular < 3);
    m_fSingularVertex[_f] = _singular;
}

unsigned FemInMesh::flatVertex(Face _f) const
{
    assert(m_fFlat[_f]);
    return m_fSingularVertex[_f];
}

void FemInMesh::setFlatVertex(Face _f, unsigned _singular)
{
    assert(m_fFlat && _singular < 3);
    m_fSingularVertex[_f] = _singular;
}

inline Node FemInMesh::node(Face _f, unsigned _index) const
{
    assert(_index < sizeElement());
    return m_fNodes[_f].array[_index];
}

inline void FemInMesh::setNode(Face _f, unsigned _index, Node _node)
{
    assert(_index < sizeElement());
    m_fNodes[_f].array[_index] = _node;
}

inline Node FemInMesh::singularNode(Face _f) const
{
    assert(m_fSingular[_f]);
    return m_fNodes[_f].array[m_singularIndex];
}

inline void FemInMesh::setSingularNode(Face _f, Node _node)
{
    assert(m_fSingular[_f]);
